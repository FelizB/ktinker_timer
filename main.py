...
def count_down(count):
    ...
    numbers= 0
    if count_min < 10:
        count_min = f"{numbers}{count_min}"
    if count_sec == 0:
        count_sec = f"{count_sec}{numbers}"
    elif count_sec < 10:
        count_sec= f"{numbers}{count_sec}"
...
button1.config(pady=5, padx=20, bg=YELLOW)
button1.grid(column=0, row=2)

button2 = Button(text="Reset",highlightthickness=0, command=reset_timer)
button2.config(pady=5, padx=20, bg=YELLOW)
